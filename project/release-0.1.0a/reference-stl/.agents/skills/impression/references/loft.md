...
- Treat path-driven body construction as a loft problem in the current product.
  Use `Path3D` as path truth, not as a reason to invent a separate sweep or
  pipe API in modeling tasks.
...
- you want the clearest surface-first expression of authored loft intent
...
- the path is part of the authored input, but you do not need surfaced station
  ownership directly
...
When you need a non-toy surfaced loft reference, open:

- `docs/examples/loft/real_world/loft_hourglass_vessel_example.py`

Use it as the canonical example of:

- surfaced `Loft(...)`
- explicit ambiguity inspection
- interactive `candidate_id` choice
- consumer-boundary tessellation
