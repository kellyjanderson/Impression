...
- threading mesh solids that use `make_cylinder`, `make_ngon`, or primitive CSG
  must move to surfaced thread assemblies or explicit mesh compatibility
...
### Threading And Hinges

`threading.py` and `hinges.py` contain surface assembly descriptions, but their
mesh paths still build printable solids with mesh primitives, mesh booleans,
and `MeshGroup` composition.

Target architecture:

- thread and hinge authorship produces surface assemblies or `SurfaceBody`
  output
...
5. Change primitive, text, drafting, hinge, threading, and heightmap authored
...
### Candidate Spec: Thread Surface Representation Defaults
...
### Candidate Spec: Thread Assembly To SurfaceBody Lowering
...
### Candidate Spec: Thread Mesh Compatibility And Quality Budget

Discovery purpose:
- Keep thread mesh generation as explicit compatibility while preserving quality and triangle-budget behavior.

Responsibilities:
- Functions/methods:
  - `_build_thread_mesh`
  - explicit thread mesh compatibility helper
  - quality budget check
- Data structures/models:
  - mesh quality estimate
  - mesh compatibility result
- Dependencies/services:
  - `threading.py`
  - mesh quality utilities
- Returns/outputs/signals:
  - explicit mesh output
...
- UI fields/elements:
  - not applicable
- Reusable code plan:
  - Existing code reused as-is: current mesh builder and quality estimator
  - Additions to existing reusable library/module: `threading.py`
  - New reusable library/module to create: none
- Database queries/tables/migrations:
  - none
- Async/concurrency behavior:
  - none
- Destructive/write behavior:
  - quarantines mesh thread route
- Security/privacy-sensitive behavior:
  - none
- Performance-sensitive behavior:
  - quality and triangle budget behavior is the core contract
- Cross-screen reusable behavior:
  - not applicable
...
- Implementation owner/module:
  - `src/impression/modeling/threading.py`
- Chosen defaults / parameters:
  - mesh thread output is explicit compatibility only
- Test strategy:
  - tests for explicit mesh route, budget, and default surface route separation
- Data ownership:
  - mesh compatibility owns mesh-only output
- Routes:
...
Open questions / nuance discovered:
- Compatibility should keep current useful quality logic without reestablishing mesh as authored truth.
...
Split decision:
- Review for split. Cohesion reason: quality budget and mesh compatibility are one mesh-only contract.
