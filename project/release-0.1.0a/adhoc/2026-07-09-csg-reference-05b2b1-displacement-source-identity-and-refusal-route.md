# CSG Reference Spec 05b2b1: Displacement Source Identity And Refusal Route
...
Implement displacement CSG source identity and source-domain compatibility
validation, including deterministic refusal diagnostics for unsupported or
ambiguous displacement operands.
...
Count: 1 IWU.
Basis: one displacement source identity and refusal boundary.

## Scope

In scope:

- displacement source identity resolution
- source-domain compatibility checks
- deterministic refusal diagnostics
- no-hidden-mesh fallback checks for refused displacement routes

Out of scope:

- successful displacement CSG result persistence
...
- refusal portions of `RT-PATCH-CSG-010`, `RT-PATCH-CSG-013`, and
  `RT-PATCH-CSG-014`

## Verification

- displacement source identity tests
- source-domain compatibility tests
...
- Routes: displacement operands to source-identity validation/refusal route
...
| --- | --- | --- |
| 1 | Split from displacement parent. | Candidate became one source/refusal leaf. |
...
- No split required. Cohesion reason: one source identity and refusal route
  boundary.

## Acceptance

This spec is complete when displacement routes can validate source identity and
refuse unsupported source-domain states deterministically without mesh fallback.
