# Surface Spec 397: Branching Loft CSG Execution Policy (v1.0)
...
Implement `Branching Loft CSG Execution Policy` as a final reference CSG gap-closure leaf.
...
This specification promotes the manifest candidate `Branching Loft CSG Execution Policy` into a final
implementation leaf.
...
Final manifest-derived implementation leaf. Current manifest score: 20.5. 1 readiness blocker(s) are carried as explicit implementation prerequisites. Split review result: Review for split. Cohesion reason: execution/decomposition policy is one branching-loft routing boundary.
...
- Rescore summary: manifest score remains 20.5; IWU count remains 1; child specifications remain none.
...
- `Branching Loft CSG Execution Policy` is implemented in the owner/module named by the manifest
