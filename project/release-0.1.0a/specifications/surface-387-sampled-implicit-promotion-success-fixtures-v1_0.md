# Surface Spec 387: Sampled Implicit Promotion Success Fixtures (v1.0)
...
Implement `Sampled Implicit Promotion Success Fixtures` as a final reference CSG gap-closure leaf.
...
This specification promotes the manifest candidate `Sampled Implicit Promotion Success Fixtures` into a final
implementation leaf.
...
Final manifest-derived implementation leaf. Current manifest score: 24.5. 1 readiness blocker(s) are carried as explicit implementation prerequisites. Split review result: Review for split. Cohesion reason: success promotion fixtures share one artifact strategy and one result-family evidence contract.
...
- `Sampled Implicit Promotion Success Fixtures` is implemented in the owner/module named by the manifest
