...
| P3 | Robust path & spline abstraction | Modeling/UX | In Progress | Polyline `Path` shipped; Bezier/spline support for downstream loft and path-based workflows is still open. |
| P4 | Loft along path | Modeling | Planned | Multi-profile loft along a path once the path/spline layer is mature. |
