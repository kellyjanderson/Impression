...
- Source purpose: Admit only the current build generation to UI-thread scene application and reject stale or post-shutdown completions before renderer mutation.
- Source responsibilities by category:
  - Functions/methods: current-result admission check, UI-thread scene apply
  - Data structures/models: applied generation state
  - Dependencies/services: Fix 01a generation state, existing scene controller
  - Returns/outputs/signals: current visible scene
  - UI surfaces/components: preview window
  - UI fields/elements: visible scene
  - Reusable code plan: extend existing preview controller/scene state
  - Database queries/tables/migrations: not applicable
  - Async/concurrency behavior: background completion is handed off; UI thread alone mutates renderer
  - Destructive/write behavior: no destructive writes
  - Security/privacy-sensitive behavior: source contents are not logged
  - Performance-sensitive behavior: generation admission is constant-time
...
Admit only the current build generation to UI-thread scene application and reject stale or post-shutdown completions before renderer mutation.

## Scope

- Owns:
  - current-generation result admission
  - UI/render-thread scene application boundary
  - stale and post-shutdown rejection

- Does not own:
  - camera, last-good, and error/recovery state, owned by Fix 01c2b
...
Pass 4 split decision: retained. Cohesion reason: one generation-admission and UI-thread scene mutation boundary.

## Implementation Routing

- Primary modules/files:
  - `src/impression/preview.py` - Admit only the current build generation to UI-thread scene application and reject stale or post-shutdown completions before renderer mutation.
...
- Tests:
  - `tests/test_preview_controller.py` generation/state tests
  - offscreen scene-apply smoke

## Chosen Defaults / Parameters

- only the current generation applies
- scene mutation occurs only on the UI/render thread
...
- Domain/service dependencies:
  ...
  - existing scene controller
- Database dependencies:
  - none
- GUI route, if applicable:
  - background completion -> UI-thread preview state handler -> preview window
- Background/concurrency route, if applicable:
  - background completion is handed off; UI thread alone mutates renderer
...
- Observable result: current visible scene
- Integration validation: `tests/test_preview_controller.py` generation/state tests; offscreen scene-apply smoke
...
- Additions to existing library/modules:
  - `src/impression/preview.py` - current-result admission check, UI-thread scene apply
...
- Functions/methods:
  - current-result admission check
  - UI-thread scene apply
...
- generation admission is constant-time

## Error And State Behavior

- stale or post-shutdown completion is discarded without renderer mutation
...
- current-generation result admission is implemented and asserted.
- UI/render-thread scene application boundary is implemented and asserted.
- stale and post-shutdown rejection is implemented and asserted.
