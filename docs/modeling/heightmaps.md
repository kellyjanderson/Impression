...
`heightmap(...)` creates a native sampled heightmap surface
payload, which keeps the output canonical before preview/export tessellation.
Tessellate the returned surface body when a mesh consumer needs triangles.
...
Displace an existing surface body by projecting a heightmap onto it.
...
- `plane`: `"xy"`, `"xz"`, or `"yz"` for planar projection sampling.
